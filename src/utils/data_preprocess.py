...
def load_datasets():
    ...
    # Orders: eliminar pedidos sin fecha de entrega (cancelados/en tránsito)
    df_orders = df_orders[df_orders['order_status'] == 'delivered'].copy()
    df_orders = df_orders.dropna(subset=['order_delivered_customer_date'])

    # Reviews: quedarse con un review por pedido (el más reciente si hay varios)
    df_reviews = df_reviews.sort_values('review_answer_timestamp', ascending=False)
    df_reviews = df_reviews.drop_duplicates(subset='order_id', keep='first')

    # Products: imputar categorías nulas con 'unknown'
    ...
    # Items: eliminar duplicados exactos
